...
websocket.on('connection', (socket) => {
    ...
    socket.on('tryJoin', (userId) => onUserJoined(userId, socket));
    socket.on('message', (message) => onMessageReceived(message, socket));
});

db.collection("messages").remove({},function(err,numberRemoved){
  ...
  try {
    ...
    if (!userId) {
      var user = db.collection('users').insert({}, (err, user) => {
        console.log("emit no use id",user.id)
        socket.emit('userJoined', user._id);
        users[socket.id] = user._id;
        _sendExistingMessages(socket);
      });
    } else {
      console.log("emit use id", userId);
